...
def test_cache_hash():
    config = CliToolConfig(
        name="test", version="1.0", version_switch="--version", schema=SchemaType.SNAPSHOT, if_os="linux"
    )
...
def test_tool_availability_result2():
    mock_last_modified = datetime(year=2023, month=1, day=15, hour=12, minute=30)
    availability_result = ToolAvailabilityResult(
        is_available=True, is_broken=True, version="2.0", last_modified=mock_last_modified
    )
...
def test_is_problem_false_not_needed_for_os():
    tool_result = ToolCheckResult(
        ...
        is_broken=False,
        last_modified=None,
        tool_config=Mock(name="mock_tool_config", schema=SchemaType.SNAPSHOT, if_os="linux"),
    )
