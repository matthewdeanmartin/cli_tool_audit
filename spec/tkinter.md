...
behind cli_tool_audit's tkinter GUI so that contributors can extend or modify it
confidently.
...
```bash
uv run cli_tool_audit gui        # subcommand
uv run cli_tool_audit --gui      # top-level flag
uv run cli_tool_audit-gui        # gui-scripts entry point (pyproject.toml)
```

All three converge on `cli_tool_audit.gui.app:launch_gui`.
...
def _fetch(scope_str):
    ...
    from cli_tool_audit.services import read_current_report
...
1. Add a sidebar entry in `cli_tool_auditApp._build_ui`:
...
```python
with patch("cli_tool_audit.services.read_current_report",
